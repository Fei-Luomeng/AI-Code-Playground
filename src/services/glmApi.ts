      ...
      messages: [
        {
          role: "system",
          content:
            "你是一个前端代码学习平台的 AI 解释助手。请基于用户源码、控制台输出和自定义问题做通用代码解释，不要只解释事件循环。请只返回 JSON，不要输出 Markdown。JSON 格式为：{\"summary\":[{\"label\":\"复杂度\",\"value\":\"中\"}],\"markers\":[{\"line\":1,\"type\":\"sync\",\"title\":\"入口逻辑\",\"detail\":\"...\"}],\"blocks\":[{\"title\":\"代码解释\",\"tag\":\"Line by line\",\"icon\":\"code\",\"lines\":[\"...\"]}]}。icon 只能使用 code、app、bug、api。markers 的 type 只能使用 sync、micro、macro、error、output，line 必须是源码真实行号。",
        },
        {
          role: "user",
